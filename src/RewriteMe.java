...
public class RewriteMe {
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category){
       return questions.stream().filter(e -> e.getCategory().equals(category)).map(e -> e.question).collect(Collectors.toList());
    ...
    public List<String> getAllAnswerOptionsDistinct(){
           return questions.stream().flatMap(e -> e.getAllAnswers().stream()).distinct().collect(Collectors.toList());
    ...
    {
        return questions.stream().filter(e -> e.getCategory().equals(c)).flatMap(e -> e.getAllAnswers().stream()).reduce(" ", (x, y) -> {if(x.length()<y.length())return y; else return x;});
